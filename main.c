...
int truncate(MINODE *mip)
{
  ...
  for(i = 0; i < 12; i++) 
  {
    if(mip->INODE.i_block[i] == 0)
    {
      continue; //No need to deallocate, don't waste runtime
    }
    bdealloc(mip->dev, mip->INODE.i_block[i]); //deallocate this direct block
  }
  //Now deallocate the indirect blocks
  if(mip->INODE.i_block[12] != 0)
  {
    get_block(mip->dev, mip->INODE.i_block[12], buffer1); //Gain access to the 256 indirect blocks pointed to by iblock[12]
    for(a = 0; a < 256; a++) //loop through all the indrect blocks
    {
      if(buffer1[a] == 0)
        {
          continue;
        }
      bdealloc(mip->dev, buffer1[a]); //deallocate the indirect block
    }
  }
  ...
  {
    get_block(mip->dev, mip->INODE.i_block[13], buffer1); //gain acess to the first level of 256 blocks in iblock[13]. Each has 256 blocks inside them...
    for(a = 0; a < 256; a++) //loop through this first level of blocks
    {
      get_block(mip->dev, buffer1[a], buffer2); //gain access to each first level blocks second level of 256 blocks
      for(b = 0; b <256; b++) //loop through that particular second level of 256 blocks. These are the double indirect blocks we want
      {
        if(buffer1[a] == 0)
        {
          continue;
        }
        bdealloc(mip->dev, buffer1[a]); //Deallocate the double indirect block
      }
    }
  }
}
